...
template = cv2.imread('E:/halcon/logo/model7/Model.bmp', cv2.COLOR_BGR2GRAY)
(tH, tW) = template.shape[:2]
print('channel:' + str(template.shape))

# 转换为灰度图片
#template = cv2.cvtColor(template, cv2.COLOR_BGR2GRAY)
# 执行边缘检测
roi = template[1140:1857, 2357:3473]
...
'''
# 遍历所有的图片寻找模板
for imagePath in glob.glob("E:/halcon/logo/model7" + "/*.bmp"):
	# 读取测试图片并将其转化为灰度图片
